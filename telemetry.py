...
from bin.connectionstatus import ConnectionStatus
from bin.quitscreen import QuitScreen
from bin.resourcemonitor import ResourceMonitor
from bin.dashboard import Dashboard
from bin.statsdashboard import StatsDashboard
from bin.errorstatus import ErrorStatus
...
class DashboardLogApp(App):
    CSS = """
        ...
        ResourceMonitor {
            ...
            height: 3;
            dock: bottom;
            border-top: round $primary-darken-2;
        }
    ...
    def __init__(self):
        ...
        # Per-session counters (formerly module-level globals).
        self.lineno = 0
        self.nodata = 0
        self.napomenutiF = 0
        self.napomenutiV = 0

        # One log file per session, kept open for the app's lifetime instead of
        # reopening it on every written line.
    ...
    def _open_session_log(self):
        ...
        while os.path.exists(f"./logs/appdatalog{date_str}_{k}.txt"):
            k += 1
        log_file = open(f"./logs/appdatalog{date_str}_{k}.txt", "a", encoding="utf-8")
        log_file.write(f"--- New session started at {datetime.now():%Y-%m-%d %H:%M:%S} ---\n")
        log_file.flush()
        return log_file
        ...
        with TabbedContent():
            with TabPane("Dashboard & Log", id="tab_dashboard"):
                with Grid(id="main_grid"):
                    ...
                    self.data_log = RichLog(highlight=False, markup=True, max_lines=150, id="data_log")
                    self.data_log.border_title = "Live Log"
                    yield self.data_log

            with TabPane("Docs", id="tab_docs"):
                with Grid(id="docs_grid"):
                    self.directory_tree = FilteredDirectoryTreeDocs("./", id="doc_tree")
                    self.directory_tree.border_title = "Docs"
                    ...
                    self.markdown_viewer.border_title = "Viewer"
            ...
            with TabPane("Config", id="tab_config"):
                with Grid(id="config_grid"):
                    ...
                    self.config_tree.border_title = "Config files"
    ...
    def handle_connection(self, config):
        ...
        if conn_type == "simulated":
            ...
            self.source = SerialSource(self.queue, conn_port, conn_baudrate)
            self.source.start()
    ...
    def handle_command(self, message):
        ...
        if message:
            if message.startswith("log "):
                ...
                try:
                    name = args[1].lower()
                    sign = {"+": 1, "-": -1}.get(args[2], 0)
                    amount = int(args[3]) * sign
                    if sign == 0:
                        self.write_log(f"unknown operation {args[2]}")
                    elif name == "v":
                        self.napomenutiV += amount
                    elif name == "f":
                        self.napomenutiF += amount
                    else:
    ...
    def start_data_stream(self):
        ...
        if self.connection_config.get("type") == "simulated":
            self.write_log("Connected to in-process simulated data source")
        elif self.connection_config.get("type") == "serial":
            self.write_log("Connected to in-process serial data source")
    ...
    def update_data(self):    
        ...
        try:
            if not self.is_connected:
                ...
                self.stats.update_stats(None, self.napomenutiF, self.napomenutiV)
                self.err_status.update_status(None, self.nodata)
                return
            ...
            while not self.queue.empty():
                ...
                if data_type[0] == "data":
                    ...
                    self.stats.update_stats(parsed_data, self.napomenutiF, self.napomenutiV)
                    self.err_status.update_status(parsed_data, self.nodata)
